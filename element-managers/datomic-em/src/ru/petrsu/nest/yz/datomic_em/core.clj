...
(ns ru.petrsu.nest.yz.datomic-em.core
  ^{:author "Vyacheslav Dimitrov"
    :doc "Implementation of the ElementManager for datomic database."}
  (:require
   (ru.petrsu.nest.yz [core :as yz] [mom-utils :as mu]))
  (:import
    (ru.petrsu.nest.yz.core ElementManager))
  (:gen-class :name ru.petrsu.nest.yz.DatomicEM
              :methods [;; JPA's element manager.
                        ^{:static true} 
                        [createDatomicElementManager 
                         [java.lang.Object] 
                         ru.petrsu.nest.yz.core.ElementManager]]))


;; Implementation of JPA's ElementManager.
(deftype DatomicElementManager [db]
  ElementManager
...
(defn ^ElementManager -createDatomicElementManager
  "Returns implementation of JPA's ElementManager."
  [db]
  (DatomicElementManager. db))
